  ...
  return (
    <Dialog defaultOpen onOpenChange={closePlansDialog}>
      <DialogContent className="max-w-screen-md h-full flex flex-col px-4 md:max-w-[50vw] md:h-auto md:max-h-[90dvh] lg:max-w-[45vw]">
          ...
          >
            {isLoading ? (
              ...
              plans?.map((plan: Plan) => (
                <div
                  key={plan.planId}
                  className="flex items-center px-3 space-x-2 border w-full rounded-md cursor-pointer"
                >
                  ...
                  <Label htmlFor={plan.planId} className="w-full h-full py-3">
                    <div className="ml-2 flex flex-col gap-1 w-full cursor-pointer font-medium">
                      ...
                      <p>
                        {plan.discountPercentage > 0 && (
                          <span className="font-light opacity-50 line-through">
                            de {toCurrency(plan.price)}
                          </span>
                        )}
        ...
        <DialogFooter>
          <Link
            href={`/payments/pix?producerId=${user.producerId}&planId=${selectedPlanId}`}
          >
