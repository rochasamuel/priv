        ...
        <SheetContent>
          ...
          <div className="flex flex-col justify-between h-full pb-4 pt-4">
            <div className="mt-2 flex flex-col">
              <SheetClose asChild>
                <Link prefetch href={"/subscriptions"} className={`${shouldHighlight("/subscriptions") ? "text-secondary" : "text-white"}`}>
                  <MobileNavItem name="Inscrições" icon={<FileSignature />} />
                </Link>
              </SheetClose>

              <SheetClose asChild>
                <Link prefetch href={"/cards"} className={`${shouldHighlight("/cards") ? "text-secondary" : "text-white"}`}>
                  <MobileNavItem name="Cartões" icon={<CreditCard />} />
                </Link>
              </SheetClose>

              <SheetClose asChild>
                <Link prefetch href={"/affiliates"} className={`${shouldHighlight("/affiliates") ? "text-secondary" : "text-white"}`}>
                  <MobileNavItem name="Afiliados" icon={<Network />} />
                </Link>
              </SheetClose>

              {isProducerProfile && <SheetClose asChild>
                <Link prefetch href={"/dashboard"} className={`${shouldHighlight("/dashboard") ? "text-secondary" : "text-white"}`}>
                  <MobileNavItem name="Dashboard" icon={<CircleDollarSign />} />
                </Link>
              </SheetClose>}

              {!isProducerProfile && <SheetClose asChild>
                <Link prefetch  href={"/become-producer"} className={`${shouldHighlight("/become-producer") ? "text-secondary" : "text-white"}`}>
              ...
              <Collapsible open={isOpen} onOpenChange={setIsOpen}>
                ...
                <CollapsibleContent className="flex flex-col pl-8">
                  {isProducerProfile && <SheetClose asChild>
                    <Link prefetch href={"/settings/plans"} className={`${shouldHighlight("/settings/plans") ? "text-secondary" : "text-white"}`}>
                      <MobileNavItem name="Planos" icon={<FileEdit />} />
                    </Link>
                  </SheetClose>}

                  {isProducerProfile && <SheetClose asChild>
                    <Link prefetch href={"/settings/bank"} className={`${shouldHighlight("/settings/bank") ? "text-secondary" : "text-white"}`}>
                      <MobileNavItem name="Dados Bancários" icon={<HandCoins />} />
                    </Link>
                  </SheetClose>}

                  <SheetClose asChild>
                    <Link prefetch href={"/settings/account"} className={`${shouldHighlight("/settings/account") ? "text-secondary" : "text-white"}`}>
                      <MobileNavItem name="Conta" icon={<UserRound />} />
                    </Link>
                  </SheetClose>

                  <SheetClose asChild>
                    <Link prefetch href={"/settings/payments"} className={`${shouldHighlight("/settings/payments") ? "text-secondary" : "text-white"}`}>
